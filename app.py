...
SOFTWARE_LICENSE_REQUIRED = os.getenv("FML_SOFTWARE_LICENSE_REQUIRED", "1").lower() in {"1", "true", "yes", "on"} or bool(SOFTWARE_LICENSE_SERVER_URL) or (bool(SOFTWARE_LICENSE_SECRET) and not SOFTWARE_LICENSE_AUTHORITY)
RATE_WINDOW = 15 * 60
LOGIN_RATE_LIMIT = 8
...
def public_user(user: sqlite3.Row) -> dict[str, Any]:
    license_key = user["license_key"] if "license_key" in user.keys() else ""
    machine_id = user["machine_id"] if "machine_id" in user.keys() else ""
    return {
        "id": user["id"],
...
def tunnel_config(user: sqlite3.Row, node: sqlite3.Row, tunnels: list[sqlite3.Row]) -> str:
    lines = [
        ...
        f'metadatas.licenseKey = "{user["license_key"] if "license_key" in user.keys() else ""}"',
        "# 必须改成本机唯一标识；首次通过鉴权后授权会绑定这个 machineId。",

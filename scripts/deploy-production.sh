...
print_summary() {
  local panel_url
  if [[ "${FML_PUBLISH_BIND}" == "0.0.0.0" ]]; then
    panel_url="http://$(hostname -I 2>/dev/null | awk '{print $1}' || hostname -f):${FML_PUBLISH_PORT}"
  fi
  cat <<SUMMARY
...
Panel:
  ...
  Public: ${PANEL_DOMAIN:+https://${PANEL_DOMAIN}}${panel_url:+${panel_url}}
...
main() {
  ...
  # Auto-detect publish bind: 0.0.0.0 if no reverse proxy, 127.0.0.1 if Nginx in front.
  if [[ -z "${PANEL_DOMAIN}" ]]; then
    FML_PUBLISH_BIND="${FML_PUBLISH_BIND:-0.0.0.0}"
    log "No PANEL_DOMAIN provided; panel will bind to ${FML_PUBLISH_BIND}:${FML_PUBLISH_PORT} (direct access)"
  else
    FML_PUBLISH_BIND="${FML_PUBLISH_BIND:-127.0.0.1}"
  fi
